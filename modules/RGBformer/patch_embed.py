...
class DTM(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.c_in = dim
        self.c_out = dim*2
        # self.dconv = nn.Conv2d(dim, dim*2, kernel_size=2, stride=2, padding=0)
        # self.dconv = DeformConv2dPack(dim, dim*2, kernel_size=2, stride=2, padding=0)
        self.norm_layer = nn.BatchNorm2d(dim*2)
        self.act_layer = nn.GELU()
        self.trans_conv = nn.ConvTranspose2d(96, 4, kernel_size=4, stride=4, padding=0)
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, H, W, C).permute(0, 3, 1, 2).contiguous()# B H W C
        x = self.trans_conv(x)
