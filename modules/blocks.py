...
from .partial_conv3 import MLPBlock
# from .ops_dcnv3.modules import DCNv3, Conv
...
class DCNv3_block(nn.Module):
    def __init__(self, inc, ouc=16, k=1, s=1, p=None, g=2, act=True):
        super().__init__()
...
if __name__ == "__main__":
    ...
    cb_blovk = CBlock_ln(dim=16).cuda()
    x = torch.Tensor(8, 16, 400, 600).cuda()
    swin = SwinTransformerBlock(dim=16, num_heads=4)
    y = cb_blovk(x)

    # device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
    # print(torch.cuda.get_device_name(device))
    a = torch.Tensor(8,3,400,600).cuda()
    dcn = DCNv3_block(3,16).cuda()
    dcn2 = DCNv3_block(16,16).cuda()
    #
    b = dcn(a)
    c = dcn2(b)
    # print(y.shape)
