...
sys.path.append('/home/zdx/paper_experiments/Illumination-Adaptive-Transformer-main')
...
class Global_pred(nn.Module):
    def __init__(self, in_channels=3, out_channels=128, num_heads=4, type='exp'):
    ...
    def forward(self, x):
        ...
        color = self.color_linear(color).squeeze(-1).view(-1, img_low.shape[-1], img_low.shape[-1]) + self.color_base

        b = img_low.shape[0]  # batch size
        img_high = img_low.permute(0, 2, 3, 1)  # (B,C,H,W) -- (B,H,W,C)
